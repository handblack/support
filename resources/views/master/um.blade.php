...
<div class="row">
    <div class="col-12">
        <div class="card">
            ...
            <h3 class="card-title"><i class="fas fa-list"></i> Gestor Unidad de Medida</h3>

            <div class="card-tools">
                <form action="{{ route('um.index') }}" method="GET" style="margin:0px;padding:0px;">
                    @csrf
                    <div class="input-group input-group-sm" style="width: 350px;">
                        <input type="text" name="q" class="form-control float-right" id="q" placeholder="Buscar..." value="{{ $q }}">
                        <div class="input-group-append">
                            <button type="submit" class="btn btn-default"><i class="fas fa-search"></i></button>
                            <a href="{{ route('um.index') }}" class="btn btn-default"><i class="fas fa-sync"></i> Actualizar </a>
                            <a href="{{ route('um.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i> Nuevo </a>
                        </div>
                    </div>
                </form>
            </div>
                ...
                <table class="table table-hover text-nowrap table-sm table-borderless" data-toggle="dataTable" data-form="deleteForm">
                    ...
                    <tbody>
                        @foreach($result as $item)
                            <form action="{{ route('um.destroy',$item->id) }}" method="POST" class="forn-inline form-delete">
                                <tr>
                                    ...
                                    <td width="80">
                                        @method('delete')
                                        @csrf
                                        <a href="{{ route('um.edit',$item->id) }}"><i class="fas fa-edit"></i> Modificar </a> | 
