        ...
        <div class="card">
            ...
            <div class="card-header">
                ...
                <div class="card-tools">
                    <form action="{{ route('received.index') }}" method="GET" style="margin:0px;padding:0px;">
                        @csrf
                        <div class="input-group input-group-sm" style="width: 250px;">
                            <input type="text" name="q" class="form-control float-right" id="q" placeholder="Buscar..." value="{{ $q }}">
                            <div class="input-group-append">
                                <button type="submit" class="btn btn-default"><i class="fas fa-search"></i></button>
                                <a href="{{ route('received.index') }}" class="btn btn-default"><i class="fas fa-sync"></i></a>
                                <a href="{{ route('received.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i> Nuevo </a>
                            </div>
                        </div>
                    </form>
                </div>
                ...
                <table class="table table-hover text-nowrap table-sm table-borderless" data-toggle="dataTable" data-form="deleteForm">
                    ...
                    <tbody>
                        @foreach($result as $item)
                            <form action="{{ route('received.destroy',$item->id) }}" method="POST" class="forn-inline form-delete">
                            <tr>
                                <td width="60">
                                    <a href="#" 
                                        class="ajax-view" 
